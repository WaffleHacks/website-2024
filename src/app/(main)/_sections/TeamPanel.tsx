...
import { type TeamMemberData, TeamMembers } from '../_components/Teams';
...
export const TeamPanel = () => {
	...
	function setPopup(e: React.MouseEvent<HTMLButtonElement, MouseEvent>, index: number) {
		...
		const contRect = containerRect.current.getBoundingClientRect();
		const midX = rect.left + rect.width / 2 - contRect.left;
		setPopupX(midX);
			...
			<Article className="mt-2 px-10 w-full mb-8 relative">
				...
				`}
					...
					{ctx.scavState && showPopupCard && (
						<div
							className="team-topcard absolute p-4 bottom-full"
							style={{
							...
							{memberToShow?.photo_name == 'tammy' && ctx.waffle.hidingSpot >= 2 ? (
								<InlineDialogSystem child={dialogChild} className="max-w-64">
									<div>
										...
										<br />
										<button
											className="bg-gray-300 px-2 py-px rounded-md"
											onClick={() => setDialogChild(1)}
										>
											Next
										</button>
									</div>
									...
									<div className="w-64 aspect-[1] relative">
										...
										{!ctx.shards.shards.includes(4) && (
											...
											>
												<img src="/assets/svgs/landing/scav/tape.svg" alt="" />
											</button>
										)}
								...
								<TeamCard
									member={memberToShow as TeamMemberData}
									style={{
										height: '50vh',
									}}
								/>
